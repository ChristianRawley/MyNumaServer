...
const getCourses = async (req, res) => {
    try {
        ...
        $('table tbody tr').each((index, element) => {
            if ($(element).find('td').length === 24) {
                ...
                if ($(nextColumn.find('td')[1]).text().length < 2) {
                    meetingTime += " & " + $(nextColumn.find('td')[1]).text().trim() + $(nextColumn.find('td')[2]).text().trim() + $(nextColumn.find('td')[3]).text().trim() + $(nextColumn.find('td')[4]).text().trim() + $(nextColumn.find('td')[5]).text().trim() + " " + $(nextColumn.find('td')[8]).text().trim();
                }
        ...
        res.status(500).send('Error scraping courses');
    }
};
